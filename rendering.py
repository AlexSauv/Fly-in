...
class MapDisplay(arcade.Window):
    ...
    def load_map_simulation(self) -> None:
        ...
        self.set_drone()
        self.precompute_drone_pos()

    def precompute_drone_pos(self):
        """
        ...
        for drone_id, path in self.simu.drones_path.items():
            for hub_name, turn in path:
                hub = self.curr_map.hubs[hub_name]
                pos_x = hub.position[0] * self.mltp + center_x
                pos_y = hub.position[1] * self.mltp + center_y

                self.drone_pos_by_turn[(drone_id, turn)] = (pos_x, pos_y)
    ...
    def on_update(self, delta_time: float) -> None:
        ...
        if not moving and self.drone_move:
            ...
            elif self.auto and not self.simturn_finished:
                has_moved = self.simu.simulation_turn()
                if has_moved:
                    self.simturn = True
                else:
                    self.simturn_finished = True
